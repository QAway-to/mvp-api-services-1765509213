...
export default function WebhookInfo({ onBitrixUrlChange }) {
  ...
  // Initialize base URL from current origin
  useEffect(() => {
  ...
  const WEBHOOK_ENDPOINTS = {
    ...
    'order/upd': {
      ...
      description: 'Webhook for order update events'
    },
    'product/upd': {
      label: 'Product Update (product/upd)',
      path: '/api/webhook/product/upd',
      description: 'Webhook for product update events'
    },
    'refund/crt': {
      label: 'Refund Create (refund/crt)',
      path: '/api/webhook/refund/crt',
      description: 'Webhook for refund creation events'
    }
  };
  ...
  const handleCopy = () => {
    ...
    const selectedUrl = baseUrl ? `${baseUrl}${selectedPath}` : '';
    if (selectedUrl) {
  ...
  const handleBitrixEndpointCopy = () => {
    ...
    const bitrixUrl = baseUrl ? `${baseUrl}${bitrixPath}` : '';
    if (bitrixUrl) {
  ...
  const selectedPath = WEBHOOK_ENDPOINTS[selectedEndpoint]?.path || '';
  const selectedUrl = baseUrl ? `${baseUrl}${selectedPath}` : '';
  const bitrixEndpointUrl = baseUrl ? `${baseUrl}${BITRIX_ENDPOINTS.bitrix.path}` : '';
      ...
      <div style={{ padding: '20px' }}>
        ...
        <div className="alert alert-info" style={{ marginTop: '20px' }}>
          ...
          <ol style={{ marginTop: '12px', paddingLeft: '20px', color: '#cbd5e1' }}>
            ...
            <li style={{ marginBottom: '8px' }}>Create 4 webhooks with these static URLs:</li>
            <ul style={{ marginLeft: '20px', marginTop: '4px', marginBottom: '8px' }}>
              ...
              <li><strong>Order update</strong> → <code style={{fontSize: '0.85rem'}}>/api/webhook/order/upd</code></li>
              <li><strong>Product update</strong> → <code style={{fontSize: '0.85rem'}}>/api/webhook/product/upd</code></li>
              <li><strong>Refund create</strong> → <code style={{fontSize: '0.85rem'}}>/api/webhook/refund/crt</code></li>
            </ul>
